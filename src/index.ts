...
app.command("show <qr-directory>").option("--interval <ms>", "milliseconds per QR", "1500").option("--gap <ms>", "black-screen gap between QR images", "0").option("--no-loop", "stop after the final slide").option("--fullscreen", "show fullscreen hint in the player").option("--recording-mode", "maximize QRs and start fullscreen playback after confirmation").option("--chunks <indexes>", "comma-separated zero-based chunk indexes").action(async (directory, options) => show(directory, options));
