...
app.command("encode <input>").option("-o, --output <directory>", "output directory").option("--chunk-size <bytes>", "raw payload bytes per QR").option("--exclude <pattern>", "exclude entry name", (value, all: string[]) => [...all, value], []).option("--error-correction <level>", "L, M, Q, or H").option("--photo-friendly", "prioritize smartphone photo readability (500B/Q/wide margin/1024px)").option("--force", "allow a non-empty output directory").action(async (input, options) => encode(input, options));
