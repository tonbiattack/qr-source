...
export interface EncodeOptions { output?: string; chunkSize?: string; exclude: string[]; errorCorrection?: string; photoFriendly?: boolean; videoFriendly?: boolean; normal?: boolean; force?: boolean; }
...
export async function encode(input: string, options: EncodeOptions): Promise<void> {
  ...
  if (options.photoFriendly && options.videoFriendly) throw new Error("Use either --photo-friendly or --video-friendly, not both"); if (options.normal && (options.photoFriendly || options.videoFriendly)) throw new Error("--normal cannot be combined with a friendly mode"); const friendly = !options.normal; const chunkSize = Number(options.chunkSize ?? (friendly ? "500" : "800")); const maxChunkSize = friendly ? 600 : 1200; if (!Number.isInteger(chunkSize) || chunkSize < 100 || chunkSize > maxChunkSize) throw new Error(`--chunk-size must be an integer from 100 to ${maxChunkSize} bytes${friendly ? " in robust default mode" : ""}`);
  ...
  if (friendly) console.log(`${options.normal ? "Normal" : "Robust default"} mode: 500-byte default chunks, Q correction, 1080px images, and a wide quiet zone.\n`);
