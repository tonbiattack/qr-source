...
export interface EncodeOptions { output?: string; chunkSize?: string; exclude: string[]; errorCorrection?: string; photoFriendly?: boolean; force?: boolean; }
...
export async function encode(input: string, options: EncodeOptions): Promise<void> {
  ...
  const chunkSize = Number(options.chunkSize ?? (options.photoFriendly ? "500" : "800")); const maxChunkSize = options.photoFriendly ? 600 : 1200; if (!Number.isInteger(chunkSize) || chunkSize < 100 || chunkSize > maxChunkSize) throw new Error(`--chunk-size must be an integer from 100 to ${maxChunkSize} bytes${options.photoFriendly ? " in --photo-friendly mode" : ""}`);
  const level = (options.errorCorrection ?? (options.photoFriendly ? "Q" : "M")).toUpperCase(); if (!(["L", "M", "Q", "H"] as string[]).includes(level)) throw new Error("--error-correction must be L, M, Q, or H");
  ...
  if (chunks.length >= (options.photoFriendly ? 50 : 100)) console.warn(`Warning: This operation will generate ${chunks.length} QR codes.${options.photoFriendly ? " Smartphone photo transfer may be impractical." : ""}`);
  await fs.mkdir(output, { recursive: true }); console.log("Generating QR codes...\n");
  if (options.photoFriendly) console.log("Photo-friendly mode: 500-byte default chunks, Q correction, 1024px images, and a wide quiet zone.\n");
  for (const [index, chunk] of chunks.entries()) { const record: QrChunk = { version: FORMAT_VERSION, projectId, chunkIndex: index, totalChunks: chunks.length, checksum, payload: chunk.toString("base64") }; const name = `qr-${String(index + 1).padStart(4, "0")}.png`; await writeQr(JSON.stringify(record), path.join(output, name), { correction: level as "L" | "M" | "Q" | "H", photoFriendly: options.photoFriendly }); console.log(`[${index + 1}/${chunks.length}] ${name}`); }
